...
async def main():
    ...
    # Регистрируем защитное middleware
    dp.message.middleware(ProtectionMiddleware())

    # Авто-перезапуск при критических падениях
    while True:
        try:
            ...
            # Пытаемся закрыть сессию и перезапустить процесс
            try:
                await bot.session.close()
            except Exception:
                pass
            logger.info('Restarting process via os.exec')
            os.execl(sys.executable, sys.executable, *sys.argv)
